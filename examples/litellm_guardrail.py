"""Use the Laya decision service as a LiteLLM guardrail and model router.

Two hooks, both calling the service over HTTP so every proxy worker shares one model:

  * `LayaGuardrail`  -- pre-call guardrail: scores the last user message with the guard preset
                        and blocks jailbreaks / injections above a threshold.
  * `laya_pick_model` -- a plain function you can call from a custom router or a pre-call hook
                        to pick a deployment from Laya's difficulty / domain / sensitivity
                        answers, so the frontier model is only used when it is needed.

Install: pip install litellm laya      (the service itself runs separately: `laya serve`)

litellm proxy config.yaml:
    guardrails:
      - guardrail_name: laya
        litellm_params:
          guardrail: examples.litellm_guardrail.LayaGuardrail
          mode: pre_call
...
from laya.presets import guard_questions, router_questions
...
class LayaGuardrail(CustomGuardrail):  # type: ignore[misc]
    """Block prompts Laya scores as jailbreak or injection above `block_threshold`."""

    def __init__(self, laya_url: str = "http://localhost:8000", api_key: Optional[str] = None,
                 block_threshold: float = 0.8, **kwargs):
        super().__init__(**kwargs)
        self.client = LayaClient(laya_url, api_key=api_key)
    ...
    async def async_pre_call_hook(self, user_api_key_dict, cache, data: dict, call_type: str):
        ...
        if worst >= self.block_threshold:
            raise ValueError("blocked by laya guardrail: jailbreak=%.2f injection=%.2f harm=%s" % (
                a["jailbreak"]["noul"], a["prompt_injection"]["noul"], a["harm_severity"]["level"]))
        # annotate for downstream routing / logging
        data.setdefault("metadata", {})["laya_guard"] = {k: v.get("noul", v.get("score")) for k, v in a.items()}
        return data


def laya_pick_model(client: LayaClient, request_text: str,
                    tiers: Dict[str, str] = None) -> Dict[str, Any]:
    """Map Laya's routing answers to a deployment name.
